...
func (r *repository) Create(ctx context.Context, newBin models.NewBin) (models.Bin, error) {
	stmt, err := r.db.PrepareContext(ctx, "INSERT INTO bins (content) VALUES (?) RETURNING id, content, created_at")
	if err != nil {
	...
	var bin models.Bin
	err = stmt.QueryRowContext(ctx, newBin.Content).Scan(&bin.Id, &bin.Content, &bin.CreatedAt)
	if err != nil {
